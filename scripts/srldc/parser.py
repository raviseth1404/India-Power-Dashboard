"""
SRLDC Daily PSP Report parser -- MODERN format (Dec 2017 onward), the same
GRID-INDIA report family as NRLDC/WRLDC/ERLDC. Older reports (2016 - mid-2017)
use a different layout and are skipped (parser returns None).

Tables captured:
  Table 1   -> regional availability (evening peak / off-peak / day energy)
  Table 2(C)-> per-state max demand + max-requirement block + ACE max/min (times).
              SRLDC has the richest 2(C): 12 value columns, mapped by header order.
  Table 3(A)-> per-state per-station generation (header-driven, like WRLDC)
"""
import re

# South-region states, exact-match (minus parentheticals) so generation sub-rows
# like "TAMILNADU_SOLAR(...)" are not misread as the state.
SOUTH_EXACT = {
    ...
    "PONDY": "PUDUCHERRY", "PDY": "PUDUCHERRY",
    "REGION": "REGION", "SR": "REGION",
}
