"""Bulk-load NERLDC jsonl into Supabase via PostgREST upsert (idempotent)."""
import json
import sys
import time
import requests
...
TABLES = {
    "regional": ("nerldc_regional_availability", "report_date"),
    "state_demand": ("nerldc_state_demand", "report_date,state_canonical"),
    "state_generation": ("nerldc_state_generation", "report_date,state_canonical,seq"),
}
...
def load(key, table, on_conflict):
    rows = [json.loads(l) for l in open(f"nerldc_{key}.jsonl")]
