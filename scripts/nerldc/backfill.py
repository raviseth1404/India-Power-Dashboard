"""
Downloads every NERLDC Daily PSP Report over the modern range (2019-07-01 ->
today) and parses tables 1 / 2C / 3A into jsonl for Supabase. NERLDC reports
live at predictable date-based URLs (no listing); missing days 404 (skipped),
old-format PDFs are skipped by is_modern(). Process pool for parallelism.
"""
import io
import json
import sys
...
URL = "https://www.nerldc.in/wp-content/uploads/NER-PSP-REPORT-DATED-{}.pdf"
...
def worker(iso):
    ...
    try:
        r = s.get(URL.format(d.strftime("%d-%m-%Y")), timeout=60)
...
def main():
    ...
    while d <= END:
        ...
        d += timedelta(days=1)
    for t in TABLES:
        open(f"nerldc_{t}.jsonl", "w").close()
    ...
    def flush():
        for t in TABLES:
            if buffers[t]:
                with open(f"nerldc_{t}.jsonl", "a") as f:
                    for r in buffers[t]:
    ...
    if failed:
        json.dump(failed, open("nerldc_failed.json", "w"))
