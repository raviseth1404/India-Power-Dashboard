"""Assemble the parsed NRLDC jsonl (main + recovered) into one multi-sheet
...
SHEETS = {
    ...
    "State Demand Met (T2C)": ("state_demand", [
        ...
        "max_requirement_mw", "max_requirement_time", "shortage_at_max_requirement_mw",
        "demand_met_at_max_requirement_mw", "min_demand_met_mw", "min_demand_time",
...
# the recovered Dec-20 report lives in a separate json blob
RECOVERED = json.load(open("/tmp/nrldc_recovered.json"))
RECOVERED_ROWS = {"regional": [RECOVERED["reg"]], "state_demand": RECOVERED["sd"],
                  "state_generation": RECOVERED["sg"]}


def load(key):
    rows = [json.loads(l) for l in open(f"nrldc_{key}.jsonl")]
    rows += RECOVERED_ROWS.get(key, [])
...
out = "../../NRLDC_Historical_PSP_2024-2026.xlsx"
wb.save(out)
print("saved", out)
