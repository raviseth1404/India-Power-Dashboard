"""Bulk-load WRLDC jsonl into Supabase via PostgREST upsert (idempotent)."""
import json
import sys
import time
import requests
...
TABLES = {
    "regional": ("wrldc_regional_availability", "report_date"),
    "state_demand": ("wrldc_state_demand", "report_date,state_canonical"),
    "state_generation": ("wrldc_state_generation", "report_date,state_canonical,seq"),
}
...
def load(key, table, on_conflict):
    rows = [json.loads(l) for l in open(f"wrldc_{key}.jsonl")]
