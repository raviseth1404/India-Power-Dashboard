"""Bulk-load ERLDC jsonl into Supabase via PostgREST upsert (idempotent)."""
import json
import sys
import time
import requests
...
TABLES = {
    "regional": ("erldc_regional_availability", "report_date"),
    "state_demand": ("erldc_state_demand", "report_date,state_canonical"),
    "state_generation": ("erldc_state_generation", "report_date,state_canonical,seq"),
}
...
def load(key, table, on_conflict):
    rows = [json.loads(l) for l in open(f"erldc_{key}.jsonl")]
